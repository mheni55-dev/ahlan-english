...
export default function NewCoursePage() {
  ...
  function handleVideoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file) {
      ...
      if (file.size > 50 * 1024 * 1024) {
        setError("حجم الفيديو يجب أن يكون أقل من 50 ميغابايت");
        return;
      }
      setError("");
      const reader = new FileReader();
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <h1 className="text-2xl font-extrabold text-navy">إضافة دورة جديدة</h1>

      <form onSubmit={handleSubmit} className="rounded-3xl bg-white border border-border p-6 space-y-5">
        {error && (
          <div className="bg-red-50 text-red-600 p-3 rounded-2xl text-sm text-center">{error}</div>
        )}
        ...
        <div>
          ...
          <div
            ...
            className="relative w-full h-48 rounded-2xl border-2 border-dashed border-border hover:border-gold cursor-pointer transition-colors overflow-hidden bg-gray-50 flex items-center justify-center"
          >
            {thumbnailPreview ? (
              <>
                ...
                <button
                  ...
                  className="absolute top-2 left-2 w-8 h-8 rounded-full bg-red-500 text-white flex items-center justify-center hover:bg-red-600 transition-colors"
                >
              ...
              <div className="text-center">
                <svg className="w-12 h-12 mx-auto text-muted/40 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
            ...
            {videoPreview ? (
              <>
                ...
                <button
                  ...
                  className="absolute top-2 left-2 w-8 h-8 rounded-full bg-red-500 text-white flex items-center justify-center hover:bg-red-600 transition-colors z-10"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                  </svg>
        ...
        <div>
          ...
          <textarea
            ...
            required
            rows={3}
            className="w-full px-4 py-2.5 rounded-2xl border border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-gold resize-none"
          />
        </div>
        ...
        <div className="flex items-center gap-6">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              name="featured"
              type="checkbox"
              className="w-4 h-4 rounded border-border text-gold focus:ring-gold accent-gold-dark"
            />
        ...
        <div className="flex items-center gap-3 pt-2">
          <button
            type="submit"
            disabled={loading}
            className="bg-gold text-navy px-8 py-2.5 rounded-full font-bold hover:bg-gold-light transition-colors disabled:opacity-50 flex items-center gap-2"
          >
            {loading ? (
              <>
                <div className="w-4 h-4 border-2 border-navy border-t-transparent rounded-full animate-spin" />
                {uploading ? "جاري الرفع..." : "جاري الإنشاء..."}
              </>
            ) : (
              "إنشاء الدورة"
            )}
